...
#ifndef __GLCARTESIANGRAPHIC_HPP__
#define __GLCARTESIANGRAPHIC_HPP__
...
public:
	...
	void AddDataPair(boost::shared_ptr<DataPair<> > _ptrDP) {
		boost::mutex::scoped_lock l(m_timed_pair_list_mtx);
		m_timed_pair_list.push_back(_ptrDP);
	}

	void DeleteDataPair(boost::shared_ptr<DataPair<> > _ptrDP) {
		boost::mutex::scoped_lock l(m_timed_pair_list_mtx);
		m_timed_pair_list.remove(_ptrDP);
	}
...
private:
	...
	float m_border;
	boost::mutex m_mouse_mtx;
	float m_mouse_x, m_mouse_y;
	boost::mutex m_timed_pair_list_mtx;
	std::list< boost::shared_ptr<DataPair<> > > m_timed_pair_list;
};
